...
`mock-expect` helps to migrate mocks generated by `mockery` from the string-based
`testify/mock` API to [Expecter Structs](https://vektra.github.io/mockery/latest/template/testify/#expecter-structs).

With `On`, the mocked method name is a string and its arguments are variadic. The Go compiler therefore cannot verify
that the method exists or that the expectation has the correct signature. Typos and stale expectations are often
discovered only when the test runs.
